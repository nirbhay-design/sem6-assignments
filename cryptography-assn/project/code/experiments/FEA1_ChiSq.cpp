...
int main(int argc, char** argv) {
    ...
    std::vector<double> thresholds {16.511,17.717,19.694,22.494,26.364,30.747};
    std::vector<size_t> data       {    16,    18,    20,    22,    24,    26};
    std::vector<size_t> trials     {   512,   512,   512,   512,   512,   512};
    ...
    for(size_t i = 0; i < data.size(); ++i) {
        ...
        for(size_t k = 0; k < trials[i]; ++k) {
            ...
            const double x = FEA1_chiSqDistinguisher(ctx, mask, N);
            if(x > t)
                ++nb_success;
