...
def train(model,train_load,val_loader,optimizer,lossfunction,n_epochs):
...
def train_model():
    ...
    history = train(torchmodel,train_loader,val_loader,optimizer,lossfunction,n_epochs=config['epochs'])
    loss_acc_curve(history,config['images_save_path'])

    test_acc = evaluate(torchmodel,test_loader,name='test')
    print('test_accuracy',test_acc)

    if config['model_'] == 'resnet':
        torch.save(torchmodel.state_dict(),config['saved_model_resnet_normal'])
    elif config['model_'] == 'vgg':
        torch.save(torchmodel.state_dict(),config['saved_model_vgg_normal'])
